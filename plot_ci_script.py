...
def plot_ci(alt_ci_nk_list, nk_list, x_list, metric, distortion, dataset, col, base_path):
    ...
    for idx, nk in enumerate(nk_list):
        if len(alt_ci_nk_list) > idx:
            ci_lower = [x[0][0] for x in alt_ci_nk_list[idx]]
            ci_upper = [x[0][1] for x in alt_ci_nk_list[idx]]
...
for distortion in tqdm(distortion_values, desc="Plotting CIs"):
    ...
    for i, nks in tqdm(enumerate(params_list), desc="Calculating CIs", leave=False):
        ...
        for n_items, k_responses in tqdm(nks[:35], desc="Processing", leave=False):
            ...
            for metric in metrics_list:
                ...
                null_ci, alt_ci = calculate_ci(gamma_null_scores), calculate_ci(gamma_alt_scores)
                ...
                if metric in alt_ci_dict and isinstance(alt_ci_dict[metric], list):
                    ...
                    metric_scores_dict[metric].append((n_items, k_responses, null_scores, alt_scores))
                else:
                    alt_ci_dict[metric] = [(alt_ci, np.mean(gamma_alt_scores))]
                    metric_scores_dict[metric] = [(n_items, k_responses, null_scores, alt_scores)]
        ...
        for metric in alt_ci_dict:
            if metric in alt_ci_nk_dict and isinstance(alt_ci_nk_dict[metric], list):
                ...
                scores_dict[metric].append(metric_scores_dict[metric])
            else:
                alt_ci_nk_dict[metric] = [alt_ci_dict[metric]]
                scores_dict[metric] = [metric_scores_dict[metric]]
    ...
    write_data_to_file(alt_ci_nk_dict, f"{ci_path}/ci_level={confidence_level}_col={col}_500_dist={distortion}.pkl.lz4")
    write_data_to_file(scores_dict, f"{scores_path}/metric_scores_col={col}_500_dist={distortion}.pkl.lz4")

...
import cat_machine_contest_metrics as cm
...
class CatMachineContestMetricsTest(absltest.TestCase):
  ...
  def test_cat_accuracy_and_f1_precision_recall(self):
    ...
    acc1, acc2 = cm.cat_accuracy(human, machine1, machine2)
    self.assertAlmostEqual(acc1, 1.0)
    self.assertLess(acc2, acc1)

    f1_1, f1_2 = cm.cat_f1_score(human, machine1, machine2)
    ...
    a1, a2 = cm.cat_actual_accuracy(human_act, m1_act, m2_act)
    ...
    auc1, auc2 = cm.cat_auc(human_bin, m1_scores, m2_scores)
  ...
  def test_mean_absolute_error_and_wins(self):
    ...
    mae1, mae2 = cm.cat_mean_absolute_error(human, machine1, machine2)
    ...
    a_mae1, a_mae2 = cm.cat_actual_mean_absolute_error(human_act, m1_act, m2_act)
    self.assertAlmostEqual(a_mae1, 0.0)
    self.assertGreater(a_mae2, 0.0)

    wins1, wins2 = cm.cat_wins_mae(human, machine1, machine2)
    self.assertGreaterEqual(wins1, 0)
    self.assertGreaterEqual(wins2, 0)
    self.assertGreater(wins1, wins2)

    a_w1, a_w2 = cm.cat_actual_wins_mae(human_act, m1_act, m2_act)
    self.assertGreaterEqual(a_w1, 0)

  def test_kl_and_jsd(self):
    # For cat_kl_div and cat_jsd provide integer vote arrays (they call freq_agg).
    human_votes = np.array([[0, 0, 0], [1, 1, 1]])
    m1_votes = human_votes.copy()
    m2_votes = np.array([[0, 1, 1], [1, 1, 1]])

    kl1, kl2 = cm.cat_kl_div(human_votes, m1_votes, m2_votes)
    ...
    akt1, akt2 = cm.cat_actual_kl_div(human_prob, m1_prob, m2_prob)
    self.assertAlmostEqual(akt1, 0.0, places=6)

    jsd1, jsd2 = cm.cat_jsd(human_votes, m1_votes, m2_votes)

...
def norm_bounded_sample(
  ...
  """Sample from a specific interval via a normal distribution.

  Args:
      mean (float): The mean of the normal distribution to sample from.
      std (float): The standard deviation of the distribution to sample from.
      bound (Tuple[float, float]): A tuple of (lower_bound, upper_bound).

  Returns:
      float: A single sample guaranteed to be within the bounds.
  """
  lower, upper = bound
...
def bi_norm_bounded_sample(
  ...
  """Samples from a mixture of two normal distributions within defined bounds.
  ...
  Args:
      ...
      mixture (float): Probability (0.0 to 1.0) of choosing the first distribution.
      bounds (Tuple[float, float]): The (lower, upper) limits for the sample.

  Returns:
      float: A bounded sample from the resulting bimodal distribution.
  """
...
def binary_norm_dist(mean: float, std: float) -> float:
  """Generates a binary output based on a sample from a normal distribution.

  Args:
      mean (float): the mean of the generating normal distribution
      std (float): the standard deviation of the generating normal distribution

  Returns:
      float: Returns 1 if the sampled value is >= 0.5, otherwise returns 0.
  """
...
def amazon_distr_gen(n: int, distortion: float) -> Tuple[
    List[Callable[[], float]],
    List[Callable[[], float]],
    List[Callable[[], float]],
]:
...
def armis_distr_gen(n: int, distortion: float) -> Tuple[
    List[Callable[[], float]],
    List[Callable[[], float]],
    List[Callable[[], float]],
]:
  ...
  """
  return gen_alt_h_distrs_norm(
      lambda: bi_norm_bounded_sample(
          -0.430701, 0.418148, 1.194010, 0.525248, 0.652561, (0, 1)
      ),
      lambda: bi_norm_bounded_sample(
          -0.264113, 0.530150, 0.362404, 0.632262, 0.766390, (0, 1)
      ),
      n,
      alt_distortion=distortion,
...
def convabuse_distr_gen(n: int, distortion: float) -> Tuple[
    List[Callable[[], float]],
    List[Callable[[], float]],
    List[Callable[[], float]],
]:
...
def hs_brexit_distr_gen(n: int, distortion: float) -> Tuple[
    List[Callable[[], float]],
    List[Callable[[], float]],
    List[Callable[[], float]],
]:
  ...
  """
  return gen_alt_h_distrs_norm(
      lambda: norm_bounded_sample(-0.278260, 0.181938, (0, 1)),
      lambda: norm_bounded_sample(-0.340141, 0.408186, (0, 1)),
      n,
      alt_distortion=distortion,
      h_dist=binary_norm_dist,
  )

def md_agreement_distr_gen(n: int, distortion: float) -> Tuple[
    List[Callable[[], float]],
    List[Callable[[], float]],
    List[Callable[[], float]],
]:
...
def mhs_distr_gen(n: int, distortion: float) -> Tuple[
    List[Callable[[], float]],
    List[Callable[[], float]],
    List[Callable[[], float]],
]:
  ...
  This specific generator is based on the measuring hate speech dataset.
  Data is located here:
  hf://datasets/ucberkeley-dlab/measuring-hate-speech/measuring-hate-speech.parquet
  ...
  """
  return gen_alt_h_distrs_norm(
      lambda: norm_bounded_sample(-0.211147, 0.106442, (0, 1)),
      lambda: norm_bounded_sample(-0.243672, 0.148406, (0, 1)),
      n,
      alt_distortion=distortion,
      h_dist=likert_norm_dist,
  )
...
class GeneratorType(enum.Enum):
  ...
  ] = functools.partial(mhs_distr_gen)
